...
class TwoDFigure:
    def __init__(self, fig_name, xmin=-6.0, xmax=6.0, ymin=-2.0, ymax=4.0, size=(6, 4)):
        """
    ...
    def centerAxes(self):
        ...
        bounds = np.array([self.ax.axes.get_xlim(), self.ax.axes.get_ylim()])
...
class ThreeDFigure:
        ...
        if len(x) > 2:
            ...
            self.desc["objects"].append(
                [
                    ...
                    alpha,
                    list(pts),
                    [[int(y) for y in x] for x in triang.triangles],
                ]
            )
            # do the plotting
            self.ax.plot_trisurf(
                triang, z, color=color, alpha=alpha, linewidth=0, shade=False
            )
    ...
    def intersectionPlaneCube(self, l1):
        ...
        """
        bounds = np.array(
            [self.ax.axes.get_xlim(), self.ax.axes.get_ylim(), self.ax.axes.get_zlim()]
        )
        coefs = l1[:3]
        b = l1[3]
        points = []
        for x, y, z in itertools.product([0, 1], repeat=3):
            ...
            for i in range(3):
                ...
                isect = (
                    b
                    - np.sum(
                        [coefs[k] * bounds[k][corner[k]] for k in range(3) if k != i]
                    )
                ) / float(coefs[i])
    ...
    def plotLine(self, in_ptlist, color, type="-", alpha=1.0):
        ...
        self.ax.plot(ptlist[0, :], ptlist[1, :], type, zs=ptlist[2, :], color=color)
    ...
    def plotCube(self, pt, color="Blue"):
        ...
        for x, y, z in itertools.product([0, 1], repeat=3):
            ...
            if x == 0:
                ptlist = [
                    [endpoints[x, 0], endpoints[y, 1], endpoints[z, 2]],
